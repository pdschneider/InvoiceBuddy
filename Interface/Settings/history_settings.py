...
from Interface.Components.gui_actions import (open_selected_folders,
                                              open_workbook)
...
def create_history_tab(globals, history_tab):
    ...
    globals.history_tree = ttk.Treeview(
        history_tab,
        columns=all_columns,
        show="headings",
        selectmode="extended")
    ...
    globals.history_tree.bind(
        "<Button-1>", on_click)
    globals.history_tree.bind(
        "<B1-Motion>", lambda e: globals.history_tree.selection_add(globals.history_tree.identify_row(e.y)) if globals.history_tree.identify_row(e.y) else None)
    globals.history_tree.bind(
        "<Control-A>", lambda e: globals.history_tree.selection_set(globals.history_tree.get_children()))
    globals.history_tree.bind(
        "<Control-a>", lambda e: globals.history_tree.selection_set(globals.history_tree.get_children()))
